<template>
	<Layout>
		...
		<section class="container my-20">
			...
			<p class="max-w-4xl mb-8 text-gray-700" v-html="$static.wordPressPage.content"></p>

			<ul class="grid gap-8 my-20 grid-col-2 md:grid-cols-3 lg:grid-cols-4">
				<li v-for="(item, index) in $static.wordPressPage.acf.gallery" :key="item.id">
					<GImage @click="openGallery(index)" :src="item.downloaded" />
				</li>
			</ul>
		</section>
		<ClientOnly>
			<LightBox ref="lightbox" :media="media" :showLightBox="false" :showThumbs="false" :showCaption="true"></LightBox>
		</ClientOnly>
	</Layout>
</template>
